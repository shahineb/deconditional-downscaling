...
class BaggedGPPredictionStrategy(DefaultPredictionStrategy):
    ...
    Args:
        train_individuals (torch.Tensor): (N, d) tensor of individuals inputs used
            for training
        train_aggregate_prior_dist (gpytorch.distribution.MultivariateNormal): Aggregate
            GP prior on training samples
    ...
    def posterior_mean_correction_cache(self):
        ...
        """
        mvn = self.likelihood(self.train_prior_dist, self.bags_sizes)
    ...
    def exact_prediction(self, individuals_mean, individuals_covar,
                         individuals_to_aggregate_covar):
        ...
        Args:
            ...
            individuals_to_aggregate_covar (torch.Tensor, gpytorch.lazy.LazyTensor)

        Returns:
            type: torch.Tensor, gpytorch.lazy.LazyTensor

        """
        return (self.exact_predictive_mean(individuals_mean, individuals_to_aggregate_covar),
                self.exact_predictive_covar(individuals_covar, individuals_to_aggregate_covar))

    def exact_predictive_mean(self, individuals_mean, individuals_to_aggregate_covar):
        ...
        """
        output = (individuals_to_aggregate_covar @ self.posterior_mean_correction_cache.unsqueeze(-1)).squeeze(-1)
        output = output + individuals_mean
        return output

    def exact_predictive_covar(self, individuals_covar, individuals_to_aggregate_covar):
        ...
        if settings.fast_pred_var.off():
            output = self._compute_predictive_covar(individuals_covar, individuals_to_aggregate_covar)

        # Using cached inverse square root covariance matrix
        else:
            covar_inv_quad_form_root = self.covar_cache.matmul(individuals_to_aggregate_covar)
    ...
    def _compute_predictive_covar(self, individuals_covar, individuals_to_aggregate_covar):
